...
---
Generate fake dataset using DummyModel
	- add cache of models created, detect and set parents
		- maybe add accessors to structure in Config
		- For parenting, detect parent from config, and use last saved:
  		- Review has multiple parents, or belongs_to,
  			- one group is reference, loaded previously, not in config
  			- another is parent in config, look for last loaded for auto-association
  			- relies on data being loaded linearly
  				- parent, then children, either dfs or bfs
  				- how to test?
